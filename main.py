...
def generateGrating(win, grating, size, contrast, sf):
    rotation = calculateIfRotated()
...
def CalculateThresholdAndSensitivity(win, grating, grating_size, cycle):
    result = []
    hits = 0
    contrast = 0.5
    for i in range(18):
        trialResult = {
            "values": generateGrating(
                win,
                grating,
                grating_size,
                contrast,
        ...
        trialResult["key"] = psychopy.event.waitKeys(keyList=["left", "right", "up"])[0]
        if trialResult["key"] == "right" and trialResult["values"]["rotation"] in range(
            GratingPhases.allowedRotation_R[0], GratingPhases.allowedRotation_R[1]
        ):
